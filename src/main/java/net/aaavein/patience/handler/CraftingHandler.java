...
import net.aaavein.patience.network.CraftingExhaustionPacket;
...
public final class CraftingHandler {
    ...
    private void tickCrafting(ContainerSettings container) {
        ...
        if (currentTime < totalTime) {
            ...
            if (player != null) {
                attributeValue = player.getAttributeValue(AttributeRegistry.CRAFTING_SPEED.get());
            }
    ...
    private void completeCraft(ContainerSettings container) {
        if (config.getHunger().getExhaustionCost() > 0) {
            NetworkHandler.CHANNEL.send(PacketDistributor.SERVER.noArg(), new CraftingExhaustionPacket(config.getHunger().getExhaustionCost()));
        }
    ...
    private float calculateCraftTime(ContainerSettings container) {
        ...
        if (!outputStack.isEmpty()) {
            ResourceLocation id = ForgeRegistries.ITEMS.getKey(outputStack.getItem());
            String modId = id.getNamespace();
            String itemId = id.toString();
    ...
    private ResourceLocation getAutomaticRecipeType() {
        if (currentScreen == null) return ForgeRegistries.RECIPE_TYPES.getKey(RecipeType.CRAFTING);
        AbstractContainerMenu menu = currentScreen.getMenu();

        if (menu instanceof CraftingMenu || menu instanceof InventoryMenu) return ForgeRegistries.RECIPE_TYPES.getKey(RecipeType.CRAFTING);
        if (menu instanceof StonecutterMenu) return ForgeRegistries.RECIPE_TYPES.getKey(RecipeType.STONECUTTING);
        if (menu instanceof SmithingMenu) return ForgeRegistries.RECIPE_TYPES.getKey(RecipeType.SMITHING);
        if (menu instanceof AbstractFurnaceMenu) return ForgeRegistries.RECIPE_TYPES.getKey(RecipeType.SMELTING);

        return ForgeRegistries.RECIPE_TYPES.getKey(RecipeType.CRAFTING);
    }
        ...
        if (!settings.getByRecipe().isEmpty()) {
            try {
                if (Minecraft.getInstance().level != null) {
                    ...
                    Optional<RecipeType<?>> typeOpt = ForgeRegistries.RECIPE_TYPES.getHolder(typeId).map(holder -> holder.get());
